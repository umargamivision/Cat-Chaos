...
public class Grabber : Singleton<Grabber>
{
    ...
    private void InputThrowGrabbable()
    {
        // Check for mouse input to throw the grabbable
        if (ControlFreak2.CF2Input.GetMouseButtonDown(0) && currentGrabbable != null) // Left mouse button
        {
            Ray screenRay = playerCamera.ScreenPointToRay(ControlFreak2.CF2Input.mousePosition);

            if (Physics.Raycast(screenRay, out RaycastHit hitInfo))
            {
                // Throw toward the clicked position
