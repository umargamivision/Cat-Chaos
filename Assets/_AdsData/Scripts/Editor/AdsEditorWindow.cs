...
public class AdsEditorWindow : EditorWindow
{
    ...
    Dictionary<IDType, AdKeyData> adsKeyData = new(){
        ...
        {IDType.AdjustIAPEventTokken,new("Adjust IAP Event Tokken","AdjustIAPEventTokkenId",IsAdjust)},
    ...
    void ShowFeatures()
    {
        ...
        AddRemoveSymbol("Tenjin", "tenjin_applovin_enabled");
        AddRemoveSymbol("ADJUST", "USE_ADJUST");
        AddRemoveSymbol("GAMEANALYTICS", "USE_GAMEANALYTICS");
    ...
    private void AddRemoveSymbol(string symbolName, string symbol)
    {
        ...
        GUILayout.Label(symbolName, EditorStyles.wordWrappedLabel);
        GUILayout.Space(10);
        GUILayout.FlexibleSpace();
        string labelText = AddDefineSymbols.CheckSymbol(symbol) ? "Remove" : "Add";
        DrawButton(labelText, symbol);
        var labelStyle = new GUIStyle(EditorStyles.wordWrappedLabel)
        {
            normal = { textColor = AddDefineSymbols.CheckSymbol(symbol) ? Color.green : Color.red }
        };
        var backgroundColor = AddDefineSymbols.CheckSymbol(symbol) ? Color.green : Color.red;
        var originalBackgroundColor = GUI.backgroundColor;
        GUI.backgroundColor = backgroundColor;
        GUILayout.Space(50);
        GUILayout.BeginVertical("box");
        GUILayout.Label(AddDefineSymbols.CheckSymbol(symbol) ? "Added" : "Removed", labelStyle, GUILayout.Width(80), GUILayout.ExpandWidth(true));
        GUILayout.EndVertical();
        GUI.backgroundColor = originalBackgroundColor;
        EditorGUILayout.EndHorizontal();
    }
    private void DrawButton(string label, string symbol)
    {
    ...
    private void AdsKeysEditor()
    {
        ...
        centeredBoldLabel.normal.textColor = Color.yellow;
        var originalBackgroundColor = GUI.backgroundColor;
        GUI.backgroundColor = Color.gray;
        GUILayout.BeginVertical("box");
        GUILayout.Label("Version: " + GVMenuItems.pluginVersion, centeredBoldLabel);
        GUILayout.EndVertical();
...
{{
    ...
    {{
        #if UNITY_ANDROID
            return ""{adsKeyData[IDType.AdjustIAPEventTokken].androidKey}"";
        #elif UNITY_IOS
            return ""{adsKeyData[IDType.AdjustIAPEventTokken].iOSKey}"";
        #else
            return """";
        #endif
    }}
