...
public class AdController : MonoBehaviour
{
    ...
    [BoxGroup("AD SETTINGS", true, true)]
    [GUIColor("@adStatus == AdStatus.Test ? Color.red : Color.green")]
    ...
    private void OnValidate()
    {
        ...
        LoadAdapters();
        if (adStatus == AdStatus.Test)
        {
            if (!gameObject.name.Contains("(** Test ADs **)"))
                gameObject.name += "(** Test ADs **)";
        }
        else
            gameObject.name = gameObject.name.Replace("(** Test ADs **)", "");
    ...
    public void ShowBannerAd(BannerAdTypes type, string placement)
    {
    ...
    private void InitializeBannerAds()
    {
        if (IsBannerAdReadyToLoad())
        {
            ...
            MaxSdk.SetBannerPlacement(AdsIds.BannerAdUnitId(), "Banner");
            if (isAdaptiveBanner)
            {
    ...
    private void ShowBanner(string placement)
    {
        ...
        MaxSdk.SetBannerPlacement(AdsIds.BannerAdUnitId(), placement);
    ...
    private void OnBannerAdLoadedEvent(string adUnitId, MaxSdkBase.AdInfo adInfo)
    {
        ...
        if (showBannerOnIntialization)
        {
            ShowBannerAd(BannerAdTypes.BANNER, "BannerOnStart");
        }
    ...
    private void InitializeMRecAds()
    {
        ...
        MaxSdk.SetMRecPlacement(AdsIds.MRECBannerAdUnitId(), "MRec");
    }
    ...
    private void ShowMRec(string placement)
    {
#if USE_MAX
        if (string.IsNullOrEmpty(AdsIds.MRECBannerAdUnitId()))
        {
        ...
        MaxSdk.SetMRecPlacement(AdsIds.MRECBannerAdUnitId(), placement);
    ...
    {
        if (IsAppOpenAdReadyToLoad() && GVRemoteConfig.Use_AppOpen_Ad)
        {
    ...
    public void ShowAppLovinAppOpen(string placement)
    {
        if (!GVRemoteConfig.Use_AppOpen_Ad)
            return;
        ...
        if (MaxSdk.IsAppOpenAdReady(AdsIds.AppOpenAdUnitId_MAX()))
        {
            ...
            MaxSdk.ShowAppOpenAd(AdsIds.AppOpenAdUnitId_MAX(), placement);
        }
        else
        {
    ...
    private void OnAppOpenLoadedEvent(string arg1, MaxSdkBase.AdInfo arg2)
    {
        ...
        {
            ShowAppLovinAppOpen("AppOpenFirstTime");
            showMaxOpenAdOnSplashSecondTime = false;
        }
    ...
    private void OnApplicationPause(bool pauseStatus)
    {
        if (!pauseStatus)
        {
            ...
            ShowAppLovinAppOpen("AppOpenOnResume");
        }
    }
